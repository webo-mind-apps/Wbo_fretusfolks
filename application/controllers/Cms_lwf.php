...
error_reporting(0);
class Cms_lwf extends CI_Controller 
{
		public function __construct()
        {
				...
				($this->session->userdata('admin_login'))?'': redirect('home/index');
					...
					$this->load->model('back_end/Cms_lwf_db','cms_lwf');
	...
	public function index()
	{
		if(($this->session->userdata('admin_login')) && ($this->session->userdata('admin_type')==0 || $this->session->userdata('admin_type')==3))
		{
			...
			$data['clients']=$this->cms_lwf->get_all_clients();
			$this->load->view('admin/back_end/cms_lwf/index',$data);
		}
		else
		{
			redirect('home/index');
	...
	function new_cms_lwf()
	{
		if(($this->session->userdata('admin_login')) && ($this->session->userdata('admin_type')==0 || $this->session->userdata('admin_type')==3))
		{
			$data['active_menu']="cms";
			$data['clients']=$this->cms_lwf->get_all_clients();
			$data['state']=$this->cms_lwf->get_all_states();
			$this->load->view('admin/back_end/cms_lwf/new_cms_lwf',$data);
		}
		else
		{
			redirect('home/index');
	...
	public function save_lwf()
	{
		if(($this->session->userdata('admin_login')) && ($this->session->userdata('admin_type')==0 || $this->session->userdata('admin_type')==3))
		{
			$data=$this->cms_lwf->save_lwf();
			redirect('cms_lwf/');
		}
		else
		{
	...
	function delete_cms_lwf()
	{
		$data1=$this->cms_lwf->delete_cms_lwf();
		$data=$this->cms_lwf->search_lwf();
		if($data)
		{
			$i=1;
			foreach($data as $row)
			{
				$file_name=substr($row['path'],16);
				echo '
					<tr>
						<td>'.$i.'</td>
							...
							<div class="list-icons">
								<div class="dropdown">
									...
									<div class="dropdown-menu dropdown-menu-right">
										<a href="javascrip:void(0);" id="'.$row['id'].'" onclick="delete_cms_lwf(this.id);" class="dropdown-item"><i class="fa fa-trash"></i> Delete</a>
									</div>
								</div>
							</div>
						</td>
