			...
			if (r == true) {
				...
				jQuery.ajax({
					...
					success: function(response) {
						...
						$('#get_details').append(response);
						$("div#divLoading").removeClass('show');
						$("#dcs_approval_table").DataTable().ajax.reload(); 
					},
					error: function(xhr, ajaxOptions, thrownError) {}
				});
			}
				...
				<div class="row">
					...
					<div class="col-md-12">
						...
						<div class="card">
							...
							<table id="dcs_approval_table" class="table datatable-basic table-bordered table-striped table-hover">
								<thead>
									<tr>
				...
				<script>
					var DatatableAdvanced = function() {
						...
						var _componentDatatableAdvanced = function() {
							...
							var dataTable = $('#dcs_approval_table').DataTable({
								'processing': true,
								'serverSide': true,
								'order': [],
								'ajax': {
									'url': "<?php echo base_url() . 'Dcs_approval/get_all_data' ?>",
									'type': 'POST',
									data: {
                                            <?php echo $this->security->get_csrf_token_name();?>: '<?php echo $this->security->get_csrf_hash();?>',
                                        },
